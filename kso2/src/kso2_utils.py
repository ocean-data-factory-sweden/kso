...
def setup_data(project_path: str | Path, data: str | Path) -> Path:
    ...
    if not data_path.exists():
        raise FileNotFoundError(f"The data_path {data_path} does not exist.")
    else:
...
def setup_model(
    project_path: str | Path, weights_path: str | Path = None, model_name: str = None
) -> Path:
    ...
    if model_path:
        if not model_path.exists():
            raise FileNotFoundError(f"The weights_path {model_path} does not exist.")
        with open(yaml_path, "r", encoding="utf-8") as f:
            data = yaml.load(f, Loader=yaml.SafeLoader)
...
def training_model(project_name: str, epochs: int = 100, imgsz: int = 640):
    ...
    if project.weights_path:
        ...
        results = model.train(data=project.data, epochs=epochs, imgsz=imgsz)
