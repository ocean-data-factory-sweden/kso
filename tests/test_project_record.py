...
def test_record_rejects_a_model_name_add_model_would_not_store(project):
    with pytest.raises(ValueError):
        proj.record(
            project,
            "models",
            {"model_name": "Skane Seg", "model_path": "runs/x/weights/best.pt"},
        )


def test_add_model_and_record_address_the_same_model(project, tmp_path):
    """One identifier: the sanitised model_name, whichever method writes."""
    proj.add_model(
        project, model_path=str(tmp_path / "base.pt"), model_name="Skane Seg"
    )
    proj.record(
        project,
        "models",
        {
            "model_name": "skane_seg",
            "model_path": "runs/segment/skane_seg/weights/best.pt",
        },
    )
    models = read_yaml(project)["models"]
    assert [m["model_name"] for m in models] == ["skane_seg"]
    assert models[0]["model_path"] == "runs/segment/skane_seg/weights/best.pt"


def test_add_model_keeps_two_models_that_start_from_the_same_weights(project, tmp_path):
    base = str(tmp_path / "base.pt")
    proj.add_model(project, model_path=base, model_name="model_a")
    proj.add_model(project, model_path=base, model_name="model_b")
    assert project.model_name == "model_b", "the second name must not be replaced"
    names = [m["model_name"] for m in read_yaml(project)["models"]]
    assert names == ["model_a", "model_b"]


def test_add_model_reuses_an_entry_with_the_same_name_and_weights(project, tmp_path):
    base = str(tmp_path / "base.pt")
    proj.add_model(project, model_path=base, model_name="model_a")
    proj.add_model(project, model_path=base, model_name="model_a")
    assert len(read_yaml(project)["models"]) == 1
    assert project.model_name == "model_a"


def test_add_model_rejects_a_name_already_used_for_other_weights(project, tmp_path):
    proj.add_model(project, model_path=str(tmp_path / "a.pt"), model_name="model_a")
    with pytest.raises(ValueError):
        proj.add_model(project, model_path=str(tmp_path / "b.pt"), model_name="model_a")
    assert len(read_yaml(project)["models"]) == 1


def test_add_model_rejects_a_name_recorded_for_trained_weights(project, tmp_path):
    """A name that already points at trained weights is taken."""
    proj.record(
        project,
        "models",
        {"model_name": "model_a", "model_path": "runs/detect/model_a/weights/best.pt"},
    )
    with pytest.raises(ValueError):
        proj.add_model(
            project, model_path=str(tmp_path / "base.pt"), model_name="model_a"
        )
