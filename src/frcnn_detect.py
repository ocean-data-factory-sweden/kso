...
def run(
    ...
    # Manual fixes for FasterRCNN model object loaded from weights
    if not str(weights).endswith(".pt"):
        os.rename(
            str(weights), str(weights).replace(os.path.split(str(weights))[-1], ".pt")
        )
        weights = str(weights).replace(os.path.split(str(weights))[-1], ".pt")
    if not isinstance(model, dict):
        model.get = dict.get
        model.names = ["fish_1", "fish_2", "fish_3", "fish_4"]
        model_dict = {"model": model}
        torch.save(model_dict, weights)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        with dt[2]:
            ...
            def apply_mns(results, applyAll=False, iou_thresh=0.3):
                ...
                results_labels_modify = (
                    results_labels
                    if not applyAll
                    else torch.zeros(len(results_boxes), dtype=torch.int32)
                )

                mns_ind = batched_nms(
                    results_boxes, results_scores, results_labels_modify, iou_thresh
                )
            ...
            def add_bounding_boxes(
                ...
                for i in range(len(boxes)):
                    ...
                    print_text = (
                        ...
                        else print_text + ":{}".format(str(round(pred_score[i], 3)))
                    )
                    cv2.putText(
                        img,
                        print_text,
        ...
        for i in range(len(dataset)):
            if webcam:  # batch_size >= 1
                p, im0, frame = (
                    path[i],
                    im.squeeze(0).numpy().transpose(1, 2, 0).copy(),
                    dataset.count,
                )
                s += f'{dict(Counter(pred["labels"].cpu().numpy()))}:'
            else:
                p, im0, frame = (
                    path,
                    im.squeeze(0).numpy().transpose(1, 2, 0).copy(),
            ...
            # .cpu().numpy()
            # image_to_log = im0.transpose(1, 2, 0)
            proc_img = cv2.cvtColor(im0, cv2.COLOR_BGR2RGB)

            proc_img = add_bounding_boxes(
                proc_img,
                pred["labels"],
                pred["boxes"],
            ...
            p = Path(p)  # to Path
            save_path = str(save_dir / p.name.split(".")[0])  # im.jpg
            ...
            if True:
                ...
                for *xyxy, conf, cls in zip(
                    ...
                    if save_txt:
                        # Write to file
                        xywh = (
                            (xyxy2xywh(torch.tensor(xyxy[0]).view(1, 4)) / gn)
                            .view(-1)
                            .tolist()
                        )  # normalized xywh
                        line = (
        ...
        LOGGER.info(
            f"{s}{'' if len(pred['boxes']) else '(no detections), '}{dt[1].dt * 1E3:.1f}ms"
        )
        if c == 10:
            break

    # Print results
    t = tuple(x.t / c * 1e3 for x in dt)  # speeds per image
    LOGGER.info(
