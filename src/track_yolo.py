...
from src.examples.detectors import get_yolo_inferer
...
from src.examples.utils import write_mot_results
from src.examples.detectors import yolov8
...
def run(args):
        ...
        if r.boxes.data.shape[1] == 7:
            ...
            if yolo.predictor.source_type.webcam or args.source.endswith(VID_FORMATS):
                ...
                p = (
                    ...
                    / "mot"
                    / (Path(args.source).parent.name + ".txt")
                )
                yolo.predictor.mot_txt_path = p
    ...
    if args.save_mot:
        if hasattr(yolo.predictor, "mot_txt_path"):
...
def parse_opt():
    ...
    parser.add_argument(
        "--vid_stride", default=1, type=int, help="video frame-rate stride"
    )
